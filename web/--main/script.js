...
function generateMergedWeeklyPlan() {
  ...
  days.forEach(day => {
    ...
    for (let i = 0; i < slots; i++) {
      ...
      left.innerHTML = `<strong>${task.course || "Course"}</strong><br><small>${task.name || task.examName || "Study Session"}</small>`;

      const right = document.createElement("span");
      right.className = "plan-hour-pill";
      right.innerText = "1h";
